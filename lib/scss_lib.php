 ...
 */
function theme_walrus_css_tree_post_processor($tree, $theme) {
 ...
 * @return string
 */
function theme_walrus_get_main_scss_content($theme) {
    global $CFG;
    ...
    foreach ($iterator as $pfile) {
        if (!$pfile->isDot()) {
            ...
            if ($filename == $presetname) {
                $scss .= file_get_contents($CFG->dirroot . '/theme/walrus/scss/preset/' . $pfile);
                $presetisset = true;
            }
        }
    }
    if (!$presetisset) {
        $filename .= '.scss';
        if ($filename && ($presetfile = $fs->get_file($context->id, 'theme_walrus', 'preset', 0    , '/', $filename))) {
            $scss .= $presetfile->get_content();
        } else {
            // Safety fallback - maybe new installs etc.
            $scss .= file_get_contents($CFG->dirroot . '/theme/walrus/scss/preset/default.scss');
        }
    }
 ...
 * @return array
 */
function theme_walrus_get_pre_scss($theme) {
    global $CFG, $OUTPUT;
 ...
 * @return string
 */
function theme_walrus_get_extra_scss($theme) {
