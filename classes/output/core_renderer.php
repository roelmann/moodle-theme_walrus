...
class core_renderer extends \theme_boost\output\core_renderer {
     ...
     * Rewritten for Walrus to incorporate header images from Course Summary Files.
     * @copyright 2017 theme_walrus Richard Oelmann https://moodle.org/user/profile.php?id=480148
     * @package    theme_walrus
     *
    ...
    public function social_icons() {
        ...
        return $this->render_from_template('theme_walrus/socialicons', $socialcontext);
    ...
    public function blocksmodal($region) {
        ...
        $maintitle = get_string('defaultmodaltitle', 'theme_walrus');
        $subtitle = get_string('defaultmodaltitledesc', 'theme_walrus');
        if (isloggedin() && ISSET($COURSE->id) && $COURSE->id > 1) {
            ...
            if ($region == 'side-sliderone') {
                if (has_capability('moodle/course:viewhiddenactivities', $context)) {
                    $maintitle = get_string('staffmodal', 'theme_walrus');
                    $subtitle = get_string('staffmodaldesc', 'theme_walrus');
                    $blocksmodalusersection .= $OUTPUT->staffblocksmodal();
                } else {
                    $maintitle = get_string('studentmodal', 'theme_walrus');
                    $subtitle = get_string('studentmodaldesc', 'theme_walrus');
        ...
        return $this->render_from_template('theme_walrus/blocksmodal', $blocksmodalcontext);
    ...
    public function staffblocksmodal() {
        ...
        if (ISSET($COURSE->id) && $COURSE->id > 1) {
            $hascoursegroup = array(
                'title' => get_string('modalcoursesettings', 'theme_walrus'),
                'icon' => 'cogs'
            );
            $hasusersgroup = array(
                'title' => get_string('modalusers', 'theme_walrus'),
                'icon' => 'users'
            );
            $hasreportsgroup = array(
                'title' => get_string('modalreports', 'theme_walrus'),
                'icon' => 'id-card'
            );
            $hasothergroup = array(
                'title' => get_string('modalstaffotherlinks', 'theme_walrus'),
                'icon' => 'external-link'
            );

            $coursegrouplinks = array(
                array(
                    'name' => get_string('editcourse', 'theme_walrus'),
                    'url' => new moodle_url('/course/edit.php', array('id' => $PAGE->course->id)),
                    'icon' => 'edit'
                ),
                array(
                    'name' => get_string('resetcourse', 'theme_walrus'),
                    'url' => new moodle_url('/course/reset.php', array('id' => $PAGE->course->id)),
                    'icon' => 'reply'
                ),
                array(
                    'name' => get_string('coursebackup', 'theme_walrus'),
                    'url' => new moodle_url('/backup/backup.php', array('id' => $PAGE->course->id)),
                    'icon' => 'copy'
                ),
                array(
                    'name' => get_string('courserestore', 'theme_walrus'),
                    'url' => new moodle_url('/backup/restorefile.php', array('contextid' => $PAGE->context->id)),
                    'icon' => 'clipboard'
                ),
                array(
                    'name' => get_string('courseimport', 'theme_walrus'),
                    'url' => new moodle_url('/backup/import.php', array('id' => $PAGE->course->id)),
                    'icon' => 'clipboard'
                ),
                array(
                    'name' => get_string('courseadmin', 'theme_walrus'),
            ...
            $usersgrouplinks = array(
                array(
                    'name' => get_string('manageusers', 'theme_walrus'),
                    'url' => $newurl,
                    'icon' => 'address-book-o'
                ),
                array(
                    'name' => get_string('manualenrol', 'theme_walrus'),
                    'url' => new moodle_url('/enrol/manual/manage.php',
                        array('enrolid' => $enrolinstance, 'id' => $courseid)),
                    'icon' => 'user-plus'
                ),
                array(
                    'name' => get_string('usergroups', 'theme_walrus'),
                    'url' => new moodle_url('/group/index.php', array('id' => $courseid)),
                    'icon' => 'group'
                ),
                array(
                    'name' => get_string('enrolmentmethods', 'theme_walrus'),
            ...
            $othergrouplinks = array(
                array(
                    'name' => get_string('recyclebin', 'theme_walrus'),
                    'url' => new moodle_url('/admin/tool/recyclebin/index.php', array('contextid' => $PAGE->context->id)),
                    'icon' => 'trash-o'
                ),
            );
            for ($i = 1; $i <= 6; $i++) {
                if (strlen(theme_walrus_get_setting('stafflink' . $i . 'name')) > 0) {
                    $othergrouplinks[] = array(
                            'name' => theme_walrus_get_setting('stafflink' . $i . 'name'),
                            'url' => theme_walrus_get_setting('stafflink' . $i . 'url'),
                            'icon' => theme_walrus_get_setting('stafflink' . $i . 'icon')
                        );
                }
            }
            ...
            return $this->render_from_template('theme_walrus/staffmodal', $staffmodalcontext);
        } else {
            return '';
        }
    }
    ...
    public function studentblocksmodal() {
        ...
        if (ISSET($PAGE->course->id) && $PAGE->course->id > 1) {
            ...
            $stulinksgroup = array();
            for ($i = 1; $i <= 6; $i++) {
                if (strlen(theme_walrus_get_setting('studentlink' . $i . 'name')) > 0) {
                    $stulinksgroup[] = array(
                            'name' => theme_walrus_get_setting('studentlink' . $i . 'name'),
                            'url' => theme_walrus_get_setting('studentlink' . $i . 'url'),
                            'icon' => theme_walrus_get_setting('studentlink' . $i . 'icon')
                        );
                }
            }
            if (count($stulinksgroup) > 0 ) {
                $hasstulinksgroup = array(
                    'title' => get_string('modalstudentlinks', 'theme_walrus'),
                    'icon' => 'link'
                );
            } else {
            ...
            $hascourseinfogroup = array (
                'title' => get_string('courseinfo', 'theme_walrus'),
                'icon' => 'map'
            );
            ...
            $hascoursestaff = array (
                'title' => get_string('coursestaff', 'theme_walrus'),
                'icon' => 'users'
            );
            ...
            return $this->render_from_template('theme_walrus/studentmodal', $studentmodalcontext);
        } else {
            return '';
        }
    }
    ...
    public function edit_button(moodle_url $url) {
        ...
        if ($this->page->user_is_editing()) {
            $url->param('edit', 'off');
            $btn = 'btn-danger';
            $title = get_string('editoff' , 'theme_walrus');
            $icon = 'fa-power-off';
        } else {
            $url->param('edit', 'on');
            $btn = 'btn-success';
            $title = get_string('editon' , 'theme_walrus');
            $icon = 'fa-edit';
        }
